...
import {FormControl, FormControlLabel, IconButton, Radio, RadioGroup, TextField} from '@mui/material';
...
// e.preventDefault()
	...
	return (<>
		...
		>
			<Grid item xs={6} md={6}
			      style={{
				      width: '50%', display: (!appointments || appointments.length === 0) ? 'none' : 'contents'
			      }}>
		...
		<div style={{height: 400, width: '90%'}}>
			<Grid
				...
				spacing={1}
				style={{
					display: 'flex', justifyContent: 'flex-end', marginBottom: '10px', maxHeight: 'fitContent',
				}}
			>
				<Grid item>
					<Link to="add">
			...
			{!appointments || appointments.length === 0 ? (<Grid container direction="column"
			                                                     justifyContent="center"
			                                                     alignItems="center">
					<Grid item xs={12}>
					...
					<Grid item xs={12}>
						<img src={noAppointment} alt="noappointment" width={'75%'}/>
					</Grid>
				...
				<Table aria-label="customized table">
					<TableHead>
						<TableRow sx={{mx: 1}}>
							<StyledTableCell align="center" style={{backgroundColor: '#5856d6'}}>
								Name
							</StyledTableCell>
							<StyledTableCell align="center" style={{backgroundColor: '#5856d6'}}>
								Email
					...
					<TableBody>
						{appointments.filter(row => {
							if (searchTerm === '') {
								return row;
							} else if ((row.patientName.toLowerCase().includes(searchTerm.toLowerCase())) || (row.DateAppointment.toLowerCase().includes(searchTerm.toLowerCase())) || (row.patientPhone.includes(searchTerm))) {
								return row;
							}
						}).map((row) => (<StyledTableRow key={row._id}>
							<StyledTableCell align="center" style={{cursor:'pointer'}} onClick={() => handleOpen(row)}>
								{row.patientName}
							</StyledTableCell>
							<StyledTableCell align="center" style={{cursor:'pointer'}}  onClick={() => handleOpen(row)}>
								{row.patientEmail}
							...
							<StyledTableCell align="center"  style={{cursor:'pointer'}} onClick={() => handleOpen(row)}>
								{row.DateAppointment.replace('T',' ').slice(0, 16)}
							</StyledTableCell>
				...
				<Fade in={open}>
					<Box sx={style} component="form"  >
						...
						>
								...
								<LocalizationProvider dateAdapter={AdapterDateFns}>
									<DateTimePicker
										...
										onChange={(newValue) => {
											...
											console.log('date appointment',appointmentData);
										}}
										renderInput={(params) => (
											<TextField
												{...params}
							...
							<Grid item>
								<TextField
									defaultValue={appointmentModify.patientName}
							...
							<Grid item>
								<TextField
					defaultValue={appointmentModify.patientEmail}
									label="Email"
									variant="outlined"
									name="patientEmail"
									onChange={handleFieldChange}
							...
							<Grid item>
								<TextField
									defaultValue={appointmentModify.patientPhone}
							...
							<Grid item>
								<FormControl>
									...
									<RadioGroup
										...
										name="gender"
										defaultValue={appointmentModify.gender}
							...
							<Grid item>
								<TextField
									defaultValue={appointmentModify.patientAge}
							...
							<Grid item>
								...
								<Button variant="contained" onClick={()=>updateApp()} color="primary"
								        disabled={toShowDateError || errorEmail || nameError}
								        endIcon={<SendIcon/>}>
									Update
								</Button>
							</Grid>
